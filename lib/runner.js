...
Runner.prototype.run = function( callback ) {
  ...
  this._get3rdParties( function( err, src, thirdParties ) {
    ...
    this.log( chalk.bgWhite.black( ' Summary for this evaluation: ' ) );
    this.log( ' -> ' + src + '\n' );
    this.log(
...
Runner.prototype._run = function( options ) {
  ...
  this.wpt.runTest(
    ...
    function( error, data ) {
      ...
      this.log(
        '  DomReady : ' + chalk.yellow(
          data.response.data.median.firstView.domContentLoadedEventEnd
        ) +
        ' | LoadTime : ' + chalk.yellow(
          data.response.data.median.firstView.loadTime
        ) +
        ' | Requests : ' + chalk.yellow(
          data.response.data.median.firstView.requests[ 0 ]
        )
      );
      this.log(
        '  DomReady : ' + chalk.yellow(
          data.response.data.median.repeatView.domContentLoadedEventEnd
        ) +
        ' | LoadTime : ' + chalk.yellow(
          data.response.data.median.repeatView.loadTime
        ) +
        ' | Requests : ' + chalk.yellow(
          data.response.data.median.repeatView.requests[ 0 ]
        )
      );
