...
Runner.prototype._emitData = function( data ) {
  this.emit( 'data', {
    allowedUrls : data.allowedUrls,
    blockedUrls : data.blockedUrls,
    firstView   : {
      bytesDocComplete : data.response.data.median.firstView.bytesInDoc,
      bytesDocLoad     : data.response.data.median.firstView.bytesIn,
      domElements      : data.response.data.median.firstView.domElements,
      ...
      render           : data.response.data.median.firstView.render,
      requests         : data.response.data.median.firstView.requests[ 0 ]

    },
    repeatView  : {
      bytesDocComplete : data.response.data.median.repeatView.bytesInDoc,
      bytesDocLoad     : data.response.data.median.repeatView.bytesIn,
      domElements      : data.response.data.median.repeatView.domElements,
      ...
      render           : data.response.data.median.repeatView.render,
...
Runner.prototype._get3rdParties = function( callback ) {
  ...
  this.emit(
    'msg',
    'Calling api with following options : ' + JSON.stringify( this.options.wpt, null, 2 )
  );
  this.wpt.runTest(
...
Runner.prototype._run = function( options ) {
  ...
  this.emit(
    'msg',
    'Calling api with following options : ' + JSON.stringify( wptOptions, null, 2 )
  );
  ...
  this.wpt.runTest(
    ...
    function( error, data ) {
      if ( error ) {
        ...
        data.allowedUrls = options.allowedThirdPartyDomain;
